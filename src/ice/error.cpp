...
#else
    std::string str(std::size_t(1024), '\0');
    if (strerror_r(code, str.data(), str.size()) == 0) {
      str.resize(strnlen(str.data(), str.size()));
      return format(str);
    }
#endif
...
std::string format(std::string what, ice::error_code ec) {
  if (ec) {
    what += ": " + ec.message();
    if (ec.type() == ice::error_type::native) {
      what += " (" + std::to_string(ec.value()) + ')';
    }
  }
  return what;
}

std::string format(std::string what, std::error_code ec) {
  if (ec) {
    what += ": " + ec.message() + " (" + std::to_string(ec.value()) + ')';
  }
  return what;
}
...
const std::error_category& native_category() noexcept {
  static const detail::native_category category;
  return category;
}

const std::error_category& system_category() noexcept {
  static const detail::system_category category;
  return category;
}

const std::error_category& domain_category() noexcept {
  static const detail::domain_category category;
  return category;
}
